...
class List extends React.Component {
  ...
  updatePost = (e) => {
    ...
    confirm({
      ...
      content: `当前操作帖子『${post.title}』将刑满释放`,
      okText: '确认',
      cancelText: '取消',
      async onOk() {
        ...
        updatePost(post.id, data).then(response => {
          if(response.data.status == 200){
            message.success(`帖子『${post.title}』成功刑满释放`)
            that.getPostList()
          }else{
            message.warning(response.data.message)
          }
      ...
      onCancel() {
        message.info(`帖子『${post.title}』未能逃离小黑屋`)
      }
    })
  }
      ...
      },
      {
        ...
        render: (text, record) => (
          <Space size="middle">
            ...
            <a style={{color: "#F2AA24"}} post={JSON.stringify(record)} onClick={this.updatePost}>释放</a>
          </Space>
        ),
      }
    ]
    ...
    return (
      <>
        <div className={styles.container}>
          <div className={styles.header}>帖子小黑屋</div>
